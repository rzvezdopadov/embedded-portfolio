...
var
  i: Integer;
begin
  Grid.Cells[1, 0] := 'Длительность';
  Grid.Cells[2, 0] := 'Период';
  Grid.Cells[3, 0] := 'Кол-во';
...
begin
  FormMain.Caption := 'Создание последовательностей генератора' + VersionProgramm;
...
begin
  ...
  begin
    try
      ...
      PanelOpenPort.Caption := 'Подключиться';
      ShowMessage('Порт успешно закрыт!');
     // ClearValueObject();
    except
      On E : Exception do
      begin
        ...
        PanelOpenPort.Caption := 'Подключиться';
        ShowMessage('Ошибка с сообщением: "' + E.Message+'"');
       // ClearValueObject();
      end;
    end;
  end else begin
    try
      ...
      PanelOpenPort.Color := clLightGreen;
      PanelOpenPort.Caption := 'Отключиться';
...
begin
  ...
  if (BufRX[1] = Chr(constCmdRead)) then begin
    if (seqPos = Chr(1)) then begin
      ...
      ShowMessage('Чтение программ завершенно!');
    end;
    ...
    if seqCount > 29 then begin
      Gauge.Progress := Round(30 * 100 / 30);
      ShowMessage('Запись программ завершенна!');
      Exit;
    end;
